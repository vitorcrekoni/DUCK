...
import { X, ArrowUpRight, Play, CheckCircle2, Sparkles, ShieldCheck, CreditCard } from 'lucide-react';
import { PaymentCheckoutModal } from './PaymentCheckoutModal';

interface WorkflowLipSyncModalProps {
  isOpen: boolean;
  onClose: () => void;
  whatsappUrl: string;
}

export const HOTMART_CHECKOUT_QUADRO_3 = 'https://pay.hotmart.com/I107666084W?bid=1789745526642';

export const WorkflowLipSyncModal: React.FC<WorkflowLipSyncModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  return (
    <>
        ...
        {isOpen && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-3xl rounded-2xl bg-[#0a1716] border border-teal-500/40 shadow-[0_0_50px_rgba(20,184,166,0.25)] p-5 sm:p-7 z-10 my-auto text-left"
            >
              {/* Sci-Fi HUD Corner Reticles */}
              <div className="absolute -top-1 -left-1 w-4 h-4 border-t-2 border-l-2 border-teal-400 z-20 pointer-events-none" />
              <div className="absolute -top-1 -right-1 w-4 h-4 border-t-2 border-r-2 border-teal-400 z-20 pointer-events-none" />
              <div className="absolute -bottom-1 -left-1 w-4 h-4 border-b-2 border-l-2 border-teal-400 z-20 pointer-events-none" />
              <div className="absolute -bottom-1 -right-1 w-4 h-4 border-b-2 border-r-2 border-teal-400 z-20 pointer-events-none" />

              {/* Glowing Accent */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-24 bg-teal-500/15 blur-3xl rounded-full pointer-events-none" />
              ...
              <div className="relative mt-5 rounded-xl overflow-hidden border border-rose-500/30 bg-black/90 shadow-[0_0_25px_rgba(0,0,0,0.8)] aspect-video">
                <iframe
                  id="odysee-iframe"
                  src="https://odysee.com/%24/embed/%40CREKONI%3A9%2FRunningHub-Fotorrealista-%2B18-%3A6?r=Du1MScs2qhpenuh9fhkyZkv5jEoUpS1d"
                  title="Vídeo Exemplo Workflow Fotorrealista +18"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 mt-4">
                <div className="p-3 rounded-lg bg-rose-950/30 border border-rose-500/20 text-xs">
                  <div className="flex items-center gap-1.5 text-rose-300 font-semibold mb-1">
                    ...
                    Sem Limites Criativos
                  </div>
                  <p className="text-slate-400 text-[11px] leading-normal">
                    Crie qualquer modelo virtual com a aparência, corpo e cenário que desejar.
                  </p>
                </div>
                ...
                <div className="p-3 rounded-lg bg-rose-950/30 border border-rose-500/20 text-xs">
                  ...
                  <p className="text-slate-400 text-[11px] leading-normal">
                    Adicione óculos, tatuagens, placas, brinquedos e tudo que imaginar via prompt.
                  </p>
                </div>
                ...
                <div className="p-3 rounded-lg bg-rose-950/30 border border-rose-500/20 text-xs">
                  ...
                  <p className="text-slate-400 text-[11px] leading-normal">
                    Gere fotos fotorrealistas +18 sem restrições ou filtros no ComfyUI.
                  </p>
                </div>
              </div>

              {/* Bottom Action Area with Hotmart Checkout Popup Trigger Button */}
              <div className="mt-6 pt-4 border-t border-teal-500/20 flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="text-center sm:text-left">
                  <span className="text-xs font-mono text-teal-300 flex items-center justify-center sm:justify-start gap-1.5">
                ...
                {/* Botão para abrir o popup de pagamento */}
                <button
                  type="button"
      ...
      {/* Pop-up Modal de Pagamento Hotmart */}
      <PaymentCheckoutModal
        isOpen={isCheckoutOpen}
        onClose={() => setIsCheckoutOpen(false)}
        checkoutUrl={HOTMART_CHECKOUT_QUADRO_3}
        title="Adquirir Workflow Fotorrealista +18 - Hotmart"
      />
    </>
  );
};
