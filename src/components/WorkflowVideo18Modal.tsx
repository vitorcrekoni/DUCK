...
import { X, ArrowUpRight, CheckCircle2, Flame, CreditCard, Video } from 'lucide-react';
import { PaymentCheckoutModal } from './PaymentCheckoutModal';

interface WorkflowVideo18ModalProps {
  isOpen: boolean;
  onClose: () => void;
  whatsappUrl: string;
}

export const HOTMART_CHECKOUT_QUADRO_4 = 'https://pay.hotmart.com/D107668469T?bid=1789747619342';

export const WorkflowVideo18Modal: React.FC<WorkflowVideo18ModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* Modal Card - Estilo futurista e imersivo no padrão do Quadro 3 */}
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 15 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 15 }}
              transition={{ type: 'spring', damping: 25, stiffness: 300 }}
              className="relative w-full max-w-3xl rounded-2xl bg-[#170a1a] border border-rose-500/40 shadow-[0_0_50px_rgba(244,63,94,0.25)] p-5 sm:p-7 z-10 my-auto text-left"
            >
              ...
              <div className="relative mt-5 rounded-xl overflow-hidden border border-rose-500/30 bg-black/90 shadow-[0_0_25px_rgba(0,0,0,0.8)] aspect-video">
                <iframe
                  id="odysee-iframe"
                  src="https://odysee.com/%24/embed/%40CREKONI%3A9%2FDaSiWa_Wan_v11_Video_1080x1920_CREKONI%3Af?r=Du1MScs2qhpenuh9fhkyZkv5jEoUpS1d"
                  title="Vídeo Exemplo Workflow DaSiWa_Wan v11 Video +18"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 mt-4">
                <div className="p-3 rounded-lg bg-rose-950/30 border border-rose-500/20 text-xs">
                  <div className="flex items-center gap-1.5 text-rose-300 font-semibold mb-1">
                    ...
                    Tecnologia DaSiWa Wan
                  </div>
                  <p className="text-slate-400 text-[11px] leading-normal">
                    Novo motor Wan v11 ultra rápido com difusão temporal e consistência corporal.
                  </p>
                </div>
                ...
                <div className="p-3 rounded-lg bg-rose-950/30 border border-rose-500/20 text-xs">
                  ...
                  <p className="text-slate-400 text-[11px] leading-normal">
                    Gere movimentos e vídeos íntimos sem restrições ou filtros no ComfyUI e na nuvem.
                  </p>
                </div>
                ...
                <div className="p-3 rounded-lg bg-rose-950/30 border border-rose-500/20 text-xs">
                  ...
                  <p className="text-slate-400 text-[11px] leading-normal">
                    Fluxo configurado integrado para movimentos realistas com edicoes configuraveis de FPS. e tempo de video.
                  </p>
                </div>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-rose-500/20 flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <button
                  type="button"
                  id="btn-modal-whatsapp-video-18"
      ...
      {/* Pop-up Modal de Pagamento Hotmart para o Quadro 4 */}
      <PaymentCheckoutModal
        isOpen={isCheckoutOpen}
        onClose={() => setIsCheckoutOpen(false)}
        checkoutUrl={HOTMART_CHECKOUT_QUADRO_4}
        title="Adquirir Workflow DaSiWa_Wan v11 Video +18 - Hotmart"
      />
    </>
  );
};
